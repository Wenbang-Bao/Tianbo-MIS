...
def product_management():
	if request.method == 'POST':
		...
		if cmd == 'SAVE_PRODUCT_INFO':
			...
			print ProductInfo.get_pro_from_request()	
		elif cmd == 'GET_PRODUCT_INFO':
			print 'send data'
			infos = ProductInfo.query.all()
...
def customer_management():
	if request.method == 'POST':
		...
		if cmd == 'SAVE_CUSTOMER_INFO':
			...
			print 'send data'	
			number = request.form.get('filter[number]')
			offset = request.form.get('filter[offset]')
			print number
			print offset
			end = offset+number
			infos = CustomerInfo.query.filter('id>:id').params(id=offset).limit(number)
			return jsonify([item.to_dict() for item in infos])
...
if __name__ == '__main__':
	app.run(host='192.168.254.176', port=8000, debug=True)
